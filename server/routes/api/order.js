...
router.post('/add', auth, async (req, res) => {
  try {
    ...
    let requester = await User.findById({'_id':orderDoc?.user});

    await mailgun.sendEmail(requester.email, 'order-confirmation',null, newOrder);
...
router.get('/:orderId', auth, async (req, res) => {
  try {
    ...
    requester = await User.findById({'_id':orderDoc?.cart?.user});
    let order = {
      _id: orderDoc._id,
      total: orderDoc.total,
      created: orderDoc.created,
